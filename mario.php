    ...
    function _p( $text )
{
  echo "<pre>";
    print_r($_POST);
      ...
      $sql = "INSERT INTO mreservations( user_id,  chas, datachas) VALUES (?,?,?)";
      $connection->prepare($sql)->execute([ $user_id,  $chas, $datachas]);
...
<head>
    ...
    <title>rezerviraite chas mario</title>
...
<?php
for($i=9; $i<=22; $i++){
  $sql = "SELECT * FROM mreservations WHERE chas='$i:00'";
  


  $chas = $connection->query("SELECT * FROM mreservations WHERE chas='$i:00'")->fetch(); 
